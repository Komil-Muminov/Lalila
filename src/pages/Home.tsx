...
import { ThemeType, ProductCategory } from '../types';
            ...
            <>
                ...
                {displayedProducts.map((product, index) => (
                  ...
                  >
                    <Link to={`/product/${product.id}`} className="block h-3/5 w-full relative cursor-pointer overflow-hidden">
                       ...
                       {!product.inStock && (
                         <div className="absolute top-4 right-4 bg-red-500 text-white text-xs font-bold px-3 py-1 rounded-full z-10 shadow-lg">SOLD OUT</div>
                       )}
                    </Link>
                    ...
                    <div className="p-6 flex-grow flex flex-col justify-between relative z-10">
                      ...
                      <div className="mt-4 flex gap-2">
                        ...
                        >
                          {product.inStock ? (<><Plus size={16} /> Quick Add</>) : 'Out of Stock'}
                        </button>
                        <Link to={`/product/${product.id}`} className={`p-3 rounded-xl flex items-center justify-center border transition-colors ${theme === ThemeType.SOFT ? 'border-[#292524]/20 hover:bg-[#292524]/5 text-[#292524]' : 'border-white/20 hover:bg-white/10 text-white'}`}>
                          <Search size={16} />
                        </Link>
                      </div>
                    </div>
              ...
              {totalPages > 1 && (
                <div className="flex justify-center items-center gap-6 pb-20">
                   ...
                   <span className={`text-sm font-bold tracking-widest ${theme === ThemeType.SOFT ? 'text-[#292524]' : (theme === ThemeType.MINIMAL ? 'text-black' : 'text-white')}`}>PAGE {currentPage} / {totalPages}</span>
                   <button
